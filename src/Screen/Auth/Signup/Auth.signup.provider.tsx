...
export function SignupScreenProvider({
  ...
  const handleBackbutton = useCallback(() => {
    ...
    if (step === 'EMAIL') {
      ...
      // 인증 완료 페이지: 뒤로 가기 버튼을 무시합니다.
      return true;
    } else if (step === 'PASSWORD') {
      // 비밀번호 입력 페이지: 뒤로 가기 버튼을 무시합니다.
      return true;
      ...
      if (signupState.accountType === 'KAKAO') {
        ...
        dispatch({type: 'UPDATE_STEP', payload: 'PASSWORD'});
        return true;
      }
  ...
  const onPressEmailStepBottomButton = async () => {
    ...
    const result = await axiosVerifyEmailAuthCode({email: signupState.email});
    if (result) {
