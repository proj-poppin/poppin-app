...
import {showBlackToast} from 'src/Util/toast.util';
import {ResponseWrapper} from 'src/Axios/wrapper/response.wrapper';
...
export async function handleAxiosError(param: {
  ...
  if (axios.isAxiosError(param.error)) {
    const responseData = param.error.response?.data as ResponseWrapper; // Type assertion
...
/**
 * Axios 요청 중 에러가 났을 때 status가 403일 때,
 * 로그인이 안 되어있는지 확인하고, 공통으로 사용할 에러를 지정해줍니다.
 * @author 규진
 */
export async function handleAuthCheckAxiosError(param: {
  error: AxiosError<ResponseWrapper> | any;
  errorMessage?: string;
  toastShowParams?: ToastShowParams;
}) {
  //* 발생한 에러가 AxiosError인지 검증하고 response 데이터를 가져올 수 있는지 확인합니다.
  if (axios.isAxiosError(param.error)) {
    const responseData = param.error.response?.data as ResponseWrapper;
    const statusCode = param.error.response?.status;

    //* 403 상태 코드 처리 (인증 관련 에러)
    if (statusCode === 403) {
      showBlackToast({
        ...param.toastShowParams,
        text1: '로그인이 필요한 서비스입니다. 로그인 후 다시 시도해 주세요.',
      });
      return;
    }
    ...
    if (responseData?.error?.message) {
      showBlackToast({
        ...param.toastShowParams,
        text1: responseData.error.message,
      });
      return;
    }
  }
