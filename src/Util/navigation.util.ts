...
export const navigateInAppScreen = (params: {
  ...
  // Handle missing navigation
  if (!navigation || !destination) {
    console.warn('Navigation or destination is undefined');
    return;
  }

  // Handle external links
  if (destination.outerLink) {
    Linking.openURL(destination.outerLink).catch(err =>
  ...
  //     });
  //   } catch (err) {
  //     console.error(`Failed to navigate to screen ${destination.screen}:`, err);
  //   }
  //   return;
  // }

  // 알림공지 탭 클릭시나, 백그라운드 실제 알림 클릭시 라우팅까지 처리
  if (
    destination.popupId ||
  ...
  if (onFailure) {
    ...
    console.warn('No destination matched, and no failure handler provided');
  }
};
